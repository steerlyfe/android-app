...
import com.napworks.steerlyfeapp.activitiesPackage.PostDetailActivity;
...
public class RecentlyPostContentAdapter extends RecyclerView.Adapter<RecentlyPostContentAdapter.MyViewHolder> {
    private final int screenWidth;
    private String TAG = getClass().getSimpleName();
    private final List<PostsInnerModel> recentlyPostLists;
    private final Activity activity;
    private int currentPosition = 0;

    public RecentlyPostContentAdapter(List<PostsInnerModel> recentlyPostLists, Activity activity, int screenWidth) {
        this.activity = activity;
        this.recentlyPostLists = recentlyPostLists;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        PostsInnerModel postsInnerModel = recentlyPostLists.get(position);
        RequestOptions requestOptions = new RequestOptions();
        requestOptions = requestOptions.transforms(new RoundedCorners(30));
        CommonMethods.showLog(TAG, "Image : " + postsInnerModel.getPostImage());
        Glide.with(activity).
                load(postsInnerModel.getPostImage()).
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (currentPosition >= 0) {
                PostsInnerModel postsInnerModel = recentlyPostLists.get(currentPosition);
                if (v.getId() == R.id.mainLinear) {
                    Intent intent = new Intent(activity, PostDetailActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                    intent.putExtra(MyConstants.POST_ID, postsInnerModel.getPostId());
